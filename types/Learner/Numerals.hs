...
learn cfg = do
  ...
  where
    ...
    transferRule rgl rbnf rule i =
      case liftM3 (,,) (evalRule "spellout-numbering" i rbnf) (evalRule "spellout-numbering" (i+10) rbnf) (evalRule "spellout-numbering" (i*10) rbnf) of
        Nothing  -> do putStrLn ("No rbnf found for "++cfgLangName cfg++" number "++show i)
                       return rgl
        Just (s,teen,ten)
                 -> do addRule rgl rule (App (App (App (Vr (identS "mkDigit")) (K s)) (K teen)) (K ten))
